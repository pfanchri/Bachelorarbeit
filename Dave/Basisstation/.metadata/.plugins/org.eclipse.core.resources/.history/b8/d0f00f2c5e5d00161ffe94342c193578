...
void tda5340_init(void) {
	...
	invalid += tda5340_command(WRITE_TO_CHIP, CLKOUT0, 0x6E); //Clock Divider Register 0
	invalid += tda5340_command(WRITE_TO_CHIP, CLKOUT2, 0x00); //Clock Divider Register 2

	/* General configuration register set */
	invalid += tda5340_command(WRITE_TO_CHIP, PPCFG0, 0xFF);                /**< Pin configuration */
	invalid += tda5340_command(WRITE_TO_CHIP, PPCFG1, 0xF2);                /**< Pin configuration */
	invalid += tda5340_command(WRITE_TO_CHIP, PPCFG2, 0x40);                /**< Pin configuration */
	invalid += tda5340_command(WRITE_TO_CHIP, RSSICFG, 0x11);               /**< RSSI Configuration register */
	invalid += tda5340_command(WRITE_TO_CHIP, RSSIOFFS, RSSI_OFFSET_VALUE); /**< RSSI offset register */
	invalid += tda5340_command(WRITE_TO_CHIP, RSSISLOPE, RSSI_SLOPE_VALUE); /**< RSSI slope register */
	invalid += tda5340_command(WRITE_TO_CHIP, ADCINSEL, 0x00);              /**< ADC Input Selection Register */
	invalid += tda5340_command(WRITE_TO_CHIP, IM0, 0xF0);                   /**< Interruption enable mask */
	invalid += tda5340_command(WRITE_TO_CHIP, IM1, 0xFF);                   /**< Interruption enable mask */
	invalid += tda5340_command(WRITE_TO_CHIP, IM2, 0xB8);                   /**< Interruption enable mask */
	invalid += tda5340_command(WRITE_TO_CHIP, RXC, 0x84);                   /**< RX control register */
	invalid += tda5340_command(WRITE_TO_CHIP, TXC, 0x27);                   /**< TX control register */
	invalid += tda5340_command(WRITE_TO_CHIP, TXFIFOAEL, 0x20);

	#if (RX_MODE == SELF_POLLING_MODE)
		// Self Polling Mode configuration
		invalid += tda5340_command(WRITE_TO_CHIP, SPMC, 0x09); // SPM control register
		invalid += tda5340_command(WRITE_TO_CHIP, SPMRT, 0x0A);
		invalid += tda5340_command(WRITE_TO_CHIP, SPMOFFT0, 0x2C);
		invalid += tda5340_command(WRITE_TO_CHIP, SPMOFFT1, 0x01);
		invalid += tda5340_command(WRITE_TO_CHIP, SPMONTA0, 0x1D);
		invalid += tda5340_command(WRITE_TO_CHIP, SPMONTA1, 0x00);
	#endif
	...
	tda5340_command(WRITE_TO_CHIP, A_PLLFRAC0C1, 0x8C); //PLL Fractional Division Ratio Register 0 Channel 1
	tda5340_command(WRITE_TO_CHIP, A_PLLFRAC1C1, 0x18); //PLL Fractional Division Ratio Register 1 Channel 1
	tda5340_command(WRITE_TO_CHIP, A_PLLFRAC2C1, 0x11); //PLL Fractional Division Ratio Register 2 Channel 1

	// Wake up control
	invalid += tda5340_command(WRITE_TO_CHIP, A_WURSSITH1, 0x80);

	// Config B for TX
	tda5340_set_page(1);
	invalid += tda5340_command(WRITE_TO_CHIP, TXCHNL, 0x00);      /**< TX channel selection */
	invalid += tda5340_command(WRITE_TO_CHIP, A_TXRF, 0x00);      /**< Duty cycle control selection */
	invalid += tda5340_command(WRITE_TO_CHIP, A_TXCFG, 0x40);     /**< TX configuration register */
	invalid += tda5340_command(WRITE_TO_CHIP, A_TXBDRDIV0, 0x76); /**< TX baudrate divider register 0. Fsys = 21948KHz */
	invalid += tda5340_command(WRITE_TO_CHIP, A_TXBDRDIV1, 0x04); /**< TX baudrate divider register 1. Fsys = 21948KHz */
	invalid += tda5340_command(WRITE_TO_CHIP, A_TXCHOFFS0, 0x00); /**< TX Channel Offset Register 0 */
	invalid += tda5340_command(WRITE_TO_CHIP, A_TXCHOFFS1, 0x00); /**< TX Channel Offset Register 1 */
	invalid += tda5340_command(WRITE_TO_CHIP, A_TXDSHCFG0, 0x00); /**< TX data shaping configuration register 0. ASK sloping division */
	invalid += tda5340_command(WRITE_TO_CHIP, A_TXDSHCFG1, 0x00); /**< TX data shaping configuration register 1. Gaussian filter division */
	invalid += tda5340_command(WRITE_TO_CHIP, A_TXDSHCFG2, 0x00); /**< TX data shaping configuration register 2. Gaussian filter division. ASK sloping division */
	invalid += tda5340_command(WRITE_TO_CHIP, A_TXPOWER0, 0x00);  /**< TX power configuration register 0 */
	invalid += tda5340_command(WRITE_TO_CHIP, A_TXPOWER1, 0x1F);  /**< TX power configuration register 1 */
	invalid += tda5340_command(WRITE_TO_CHIP, A_TXFDEV, 0xD7);    /**< TX frequency deviation register */

	// Setting TX-frequency to 868.0 MHz
	invalid += tda5340_command(WRITE_TO_CHIP, A_PLLINTC1, 0x67);   /**< PLL multi modulus divider integer value channel 1 */
	invalid += tda5340_command(WRITE_TO_CHIP, A_PLLFRAC0C1, 0xD1); /**< PLL fractional division ratio channel 1 */
	invalid += tda5340_command(WRITE_TO_CHIP, A_PLLFRAC1C1, 0x7E); /**< PLL fractional division ratio channel 1 */
	invalid += tda5340_command(WRITE_TO_CHIP, A_PLLFRAC2C1, 0x11); /**< PLL fractional division ratio channel 1 */
	...
	// Setting TX-frequency to 868.0 MHz
	invalid += tda5340_command(WRITE_TO_CHIP, A_PLLINTC1, 0x67);   /**< PLL multi modulus divider integer value channel 1 */
	invalid += tda5340_command(WRITE_TO_CHIP, A_PLLFRAC0C1, 0x95); /**< PLL fractional division ratio channel 1 */
	invalid += tda5340_command(WRITE_TO_CHIP, A_PLLFRAC1C1, 0x09); /**< PLL fractional division ratio channel 1 */
	invalid += tda5340_command(WRITE_TO_CHIP, A_PLLFRAC2C1, 0x10); /**< PLL fractional division ratio channel 1 */
