...
void tda5340_gpio_init(uint8_t device_number) {
	...
	if (device_number == TDA_ALL || device_number == TDA1) {
		const XMC_ERU_ETL_CONFIG_t PP2_ETL_CONFIG = {
				.input_b = XMC_ERU_ETL_INPUT_B0,	//set here Interrupt Input
	...
	if (device_number == TDA_ALL || device_number == TDA5) {
		const XMC_ERU_ETL_CONFIG_t PP2_ETL_CONFIG = {
				.input_b = XMC_ERU_ETL_INPUT_A1,	//set here Interrupt Input
	...
	if (device_number == TDA_ALL || device_number == TDA6) {
		const XMC_ERU_ETL_CONFIG_t PP2_ETL_CONFIG = {
				.input_b = XMC_ERU_ETL_INPUT_A0,	//set here Interrupt Input
				.source = XMC_ERU_ETL_SOURCE_A,	//and Interrupt source (from Ref.Manual)
				.edge_detection = XMC_ERU_ETL_EDGE_DETECTION_FALLING,
				.status_flag_mode = XMC_ERU_ETL_STATUS_FLAG_MODE_HWCTRL,
				.enable_output_trigger = 1, .output_trigger_channel =
...
//TODO: auf mehrer TDAs anpassen
uint8_t tda5340_transmit(uint8_t device_number, uint8_t config, int8_t *data,
		uint8_t length) {
