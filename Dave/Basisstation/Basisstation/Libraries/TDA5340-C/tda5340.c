...
void tda5340_gpio_init(uint8_t device_number) {
	...
	if (device_number == TDA_ALL || device_number == TDA6) {
		...
		XMC_GPIO_EnableDigitalInput(PORT_PP1_TDA_1, PIN_PP1_TDA_1);				//da PP2 von TDA6 mit Brücke auf PP11 gelegt
				XMC_GPIO_CONFIG_t TDA_PP2_new_CONFIG = { .mode = XMC_GPIO_MODE_INPUT_PULL_UP };
				XMC_GPIO_Init(PORT_PP1_TDA_1, PIN_PP1_TDA_1, &TDA_PP2_new_CONFIG);
	}

	//	configure Interrupts (depending of device number)
	//TODO: Einstellen der Pins/PORTS ERU OGU etc
	//		for TDA1
	if (device_number == TDA_ALL || device_number == TDA1) {
		const XMC_ERU_ETL_CONFIG_t PP2_ETL_CONFIG_1 = { .input_b = XMC_ERU_ETL_INPUT_B0,	//set here Interrupt Input
		...
		XMC_ERU_ETL_Init(ERU1_ETL3, &PP2_ETL_CONFIG_1);
		const XMC_ERU_OGU_CONFIG_t PP2_EVENT_DETECTION_CONFIG_1 = { .service_request =
				XMC_ERU_OGU_SERVICE_REQUEST_ON_TRIGGER };
		XMC_ERU_OGU_Init(ERU1_OGU0, &PP2_EVENT_DETECTION_CONFIG_1);
	}
	//		for TDA2
	if (device_number == TDA_ALL || device_number == TDA2) {
		const XMC_ERU_ETL_CONFIG_t PP2_ETL_CONFIG_2 = { .input_b = XMC_ERU_ETL_INPUT_B0,	//set here Interrupt Input
				.source = XMC_ERU_ETL_SOURCE_B,	//and Interrupt source (from Ref.Manual)
				.edge_detection = XMC_ERU_ETL_EDGE_DETECTION_FALLING, .status_flag_mode =
						XMC_ERU_ETL_STATUS_FLAG_MODE_HWCTRL, .enable_output_trigger = 1, .output_trigger_channel =
						XMC_ERU_ETL_OUTPUT_TRIGGER_CHANNEL0 /* OGU0 */
		};
		XMC_ERU_ETL_Init(ERU0_ETL3, &PP2_ETL_CONFIG_2);
		const XMC_ERU_OGU_CONFIG_t PP2_EVENT_DETECTION_CONFIG_2 = { .service_request =
				XMC_ERU_OGU_SERVICE_REQUEST_ON_TRIGGER };
	...
	if (device_number == TDA_ALL || device_number == TDA3 /*|| device_number == TDA6*/) {
		const XMC_ERU_ETL_CONFIG_t PP2_ETL_CONFIG_3 = { //.input_a = XMC_ERU_ETL_INPUT_A0,
				...
				.source = XMC_ERU_ETL_SOURCE_B,//A_AND_B,	//and Interrupt source (from Ref.Manual)
	...
	//		for TDA4
	if (device_number == TDA_ALL || device_number == TDA4) {
		const XMC_ERU_ETL_CONFIG_t PP2_ETL_CONFIG_4 = { .input_b = XMC_ERU_ETL_INPUT_B0,	//set here Interrupt Input
				.source = XMC_ERU_ETL_SOURCE_B,	//and Interrupt source (from Ref.Manual)
				.edge_detection = XMC_ERU_ETL_EDGE_DETECTION_FALLING, .status_flag_mode =
						XMC_ERU_ETL_STATUS_FLAG_MODE_HWCTRL, .enable_output_trigger = 1, .output_trigger_channel =
						XMC_ERU_ETL_OUTPUT_TRIGGER_CHANNEL1 /* OGU1 */
		};
		XMC_ERU_ETL_Init(ERU1_ETL0, &PP2_ETL_CONFIG_4);
		const XMC_ERU_OGU_CONFIG_t PP2_EVENT_DETECTION_CONFIG_4 = { .service_request =
				XMC_ERU_OGU_SERVICE_REQUEST_ON_TRIGGER };
		XMC_ERU_OGU_Init(ERU1_OGU1, &PP2_EVENT_DETECTION_CONFIG_4);

	}
	//		for TDA5
	if (device_number == TDA_ALL || device_number == TDA5) {
		const XMC_ERU_ETL_CONFIG_t PP2_ETL_CONFIG_5 = { .input_a = XMC_ERU_ETL_INPUT_A1,	//set here Interrupt Input
				.source = XMC_ERU_ETL_SOURCE_A,	//and Interrupt source (from Ref.Manual)
				.edge_detection = XMC_ERU_ETL_EDGE_DETECTION_FALLING, .status_flag_mode =
						XMC_ERU_ETL_STATUS_FLAG_MODE_HWCTRL, .enable_output_trigger = 1, .output_trigger_channel =
						XMC_ERU_ETL_OUTPUT_TRIGGER_CHANNEL2 /* OGU2 */
		};
		XMC_ERU_ETL_Init(ERU0_ETL0, &PP2_ETL_CONFIG_5);
		const XMC_ERU_OGU_CONFIG_t PP2_EVENT_DETECTION_CONFIG_5 = { .service_request =
				XMC_ERU_OGU_SERVICE_REQUEST_ON_TRIGGER };
		XMC_ERU_OGU_Init(ERU0_OGU2, &PP2_EVENT_DETECTION_CONFIG_5);
	...
	//	activate Interrupts and set priority
	NVIC_SetPriority(ERU0_0_IRQn, IRQ_PRIORITY_TDA2);
	NVIC_EnableIRQ(ERU0_0_IRQn);
	//	activate Interrupts and set priority
	NVIC_SetPriority(ERU0_1_IRQn, IRQ_PRIORITY_TDA3);
	NVIC_EnableIRQ(ERU0_1_IRQn);
	//	activate Interrupts and set priority
	NVIC_SetPriority(ERU1_1_IRQn, IRQ_PRIORITY_TDA4);
...
/**
 * reads/writes to given Register
 * Param device number:	Number of TDA; TDA1...TDA6
 * Param instruction:	Instruction such as: READ_FROM_CHIP, WRITE_TO_CHIP,...
 * Param reg_address	Address of Register
 * Param *length		Data to store in Register
 * return:				SPI answer of TDA to communication
 */uint8_t tda5340_transfer(uint8_t device_number, uint8_t instruction, uint8_t reg_address, uint16_t data) {
...
/**
 * gets Data to TDA and sets TDA to transmit mode
 * Param device number:	Number of TDA; TDA1...TDA6
 * Param config:		??? TODO: was macht dies?
 * Param *data:			Array of Data to send or store Data From FIFO
 * Param *length		Length of data
 * return:				CRC error if occurs
...
/**
 * reads or writes in FIFO
 * Param device number:	Number of TDA; TDA1...TDA6
 * Param wnr:			Read/write control: 1->write to FIFO; 0->read from FIFO
 * Param *data:			Array of Data to send or store Data From FIFO
 * Param *length		Length of data
 * return:				0 when write to FIFO or CRC when read from FIFO
 */
uint8_t tda5340_fifo_rw(uint8_t device_number, uint8_t wnr, int8_t *data, uint8_t *length) {
	...
	if (wnr) {
		...
		spi_transfer(spi_master_ch, device_number, transferdata, transferdata, length_local + 10); // FIFO-Order + FIFO length + 2 Bytes Key + Payload length + Payload + 2 Bytes CRC
	}
	// Read from FIFO
	else {
...
/**
 * Sets TDA to definded mode
 * Param device number:	Number of TDA; TDA1...TDA6
 * Param mode:			mode TDA should be set to: RX_MODE, TX MODE,...
 * Param config:		Multi Configuration Section: (Run Mode Slave/Self Polling Mode/Transmit) see Chip Mode Controll register (CMC)
 * return:				void
 */
void tda5340_set_mode_and_config(uint8_t device_number, uint8_t mode, uint8_t config) {
	...
	if (mode == DEEP_SLEEP_MODE) {
		tda5340_transfer(device_number, WRITE_TO_CHIP, CMC, 0x30 | transferbit);//enable Deep Sleep Mode in CMC Register
		set_TDA_status(device_number, 0);
	} else {
		tda5340_transfer(device_number, WRITE_TO_CHIP, CMC, 0x10 | transferbit);
		set_TDA_status(device_number, 1);
	}
}
...
void tda5340_init(uint8_t device_number) {
	...
	tda5340_command(device_number, WRITE_TO_CHIP, A_PLLFRAC0C1, 0x8C); //PLL Fractional Division Ratio Register 0 Channel 1
	tda5340_command(device_number, WRITE_TO_CHIP, A_PLLFRAC1C1, 0x18); //PLL Fractional Division Ratio Register 1 Channel 1
	...
	invalid += tda5340_command(device_number, WRITE_TO_CHIP, A_PLLFRAC0C1, 0xD1); /**< PLL fractional division ratio channel 1 */
	invalid += tda5340_command(device_number, WRITE_TO_CHIP, A_PLLFRAC1C1, 0x7E); /**< PLL fractional division ratio channel 1 */
	invalid += tda5340_command(device_number, WRITE_TO_CHIP, A_PLLFRAC2C1, 0x11); /**< PLL fractional division ratio channel 1 */
