...
// CRC8 according to SENSIRION for byte "data" with register preload "crc"
uint8_t crc8_sens(uint8_t crc, uint8_t data) {
	crc ^= data;
	...
	for (uint8_t i = 8; i; i--) {
		...
		if (data) crc ^= 0x31; // 0x31 due to polynom x^8+x^5+x^4+1 and MSB first
	}
